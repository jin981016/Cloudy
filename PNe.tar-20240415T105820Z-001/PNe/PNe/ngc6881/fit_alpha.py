...
df = pd.read_csv('cngc6881_3300s.0054.txt', sep='  ',names = ['wavelength','flux']) #alpha
...
for i in range(nn):
    if 6526 < ww[i] and ww[i] < 6528.5:
        wl_he27.append(ww[i])
        fl_he27.append(ff[i])    
    elif 6559. < ww[i] and ww[i] <= 6561.:
        wl_he60.append(ww[i])
        fl_he60.append(ff[i])        
    elif 6561.2 <= ww[i] and ww[i]<= 6564.2:
        wl_ha.append(ww[i])
        fl_ha.append(ff[i])
    elif 6546.5 < ww[i] and ww[i] <6549.3:
        wl_d.append(ww[i])
        fl_d.append(ff[i])
    else:
        wl.append(ww[i])
...
for j in range(len(wl)):
    if 6542<wl[j] and wl[j]<6553.5:
        wl_r.append(wl[j])
        fl_r.append(fl[j])
    elif 6557.4<wl[j] and wl[j]<6559.5:
        wl_w.append(wl[j])
        fl_w.append(fl[j])
    elif 6564.4<wl[j] and wl[j]<6567:
        wl_w.append(wl[j])
        fl_w.append(fl[j])
...
popt1,pcov1 = curve_fit(gauss,wl_he27,fl_he27,p0=[p_he27,cwl_he27,std_he27],maxfev = 15000)
...
x2 = np.linspace(6559,np.max(wl_he60),100)

popt2,pcov2 = curve_fit(gauss,wl_he60,fl_he60,p0=[p_he60,cwl_he60,std_he60],maxfev = 15000)
...
x3 = np.linspace(np.min(wl_ha),np.max(wl_ha),100)

popt3,pcov3 = curve_fit(gauss,wl_ha,fl_ha,p0=[p_ha,cwl_ha,std_ha],maxfev = 15000)
...
# plt.plot(x3,y3,color='red', linestyle='--', label = 'Gaussian fitting')
# plt.xlim(6520,6570)
# plt.ylim(0,4.1) 
...
popt4,pcov4 = curve_fit(gauss,wl_w,fl_w,p0=[p_w,cwl_w,std_w],maxfev = 15000)
...
popt5,pcov5 = curve_fit(gauss,wl_r,fl_r,p0=[p_r,cwl_r,std_r],maxfev = 15000)
...
plt.xlabel(r'Wavelength[$\AA$]')
plt.ylabel(r'Flux[$10^{-12}$erg$s^{-1}$$cm^{2}$$\AA^{-1}$]')
plt.text(6521,0.021,'HeII 6527',fontdict={'fontsize':'10','fontweight':'bold'})
plt.annotate('',xy=(6525.7,0.013),xytext=(6524.8,0.02),arrowprops=dict(arrowstyle='->',color='black',lw=2))
...
plt.title('Raman HeII 6545 of NGC 6881')
plt.grid(linestyle=':')
plt.legend(loc = 'upper right')
plt.savefig('6545_NGC6881.png')
#plt.show()

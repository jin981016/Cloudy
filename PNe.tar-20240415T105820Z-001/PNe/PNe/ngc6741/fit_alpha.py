...
df = pd.read_csv('cngc6741_3600s.0055.txt', sep='  ',names = ['wavelength','flux']) #alpha
...
wl,fl=[],[]
for i in range(nn):
    if 6526 < ww[i] and ww[i] < 6528.5:
        wl_he27.append(ww[i])
        fl_he27.append(ff[i])    
    elif 6559.5 < ww[i] and ww[i] <= 6561.35:
        wl_he60.append(ww[i])
        fl_he60.append(ff[i])        
    elif 6563.<= ww[i] and ww[i]< 6564.5:
        wl_ha.append(ww[i])
        fl_ha.append(ff[i])
    elif 6546 < ww[i] and ww[i] <6550:
        wl_d.append(ww[i])
        fl_d.append(ff[i])
    else:
        wl.append(ww[i])
...
for j in range(len(wl)):
    if 6542<wl[j] and wl[j]<6553:
        wl_r.append(wl[j])
        fl_r.append(fl[j])
    elif 6558.4<=wl[j] and wl[j]<6559.5:
        wl_w.append(wl[j])
        fl_w.append(fl[j])
    elif 6565.5<=wl[j] and wl[j]<6566.5:
        wl_w.append(wl[j])
        fl_w.append(fl[j])
...
x1 = np.linspace(6500,6570,1000)

popt1,pcov1 = curve_fit(gauss,wl_he27,fl_he27,p0=[p_he27,cwl_he27,std_he27],maxfev = 150000)
...
popt3,pcov3 = curve_fit(gauss,wl_ha,fl_ha,p0=[p_ha,cwl_ha,std_ha],maxfev = 150000)
...
popt4,pcov4 = curve_fit(gauss,wl_w,fl_w,p0=[p_w,cwl_w,std_w],maxfev = 150000)
...
popt5,pcov5 = curve_fit(gauss,wl_r,fl_r,p0=[p_r,cwl_r,std_r],maxfev = 150000)
...
plt.xlabel(r'Wavelength[$\AA$]')
plt.ylabel(r'Flux[$10^{-12}$erg$s^{-1}$$cm^{2}$$\AA^{-1}$]')
plt.text(6530,0.125,'HeII 6527',fontdict={'fontsize':'10','fontweight':'bold'})
plt.annotate('',xy=(6530,0.1),xytext=(6533.5,0.12),arrowprops=dict(arrowstyle='->',color='black',lw=2))
plt.text(6532,0.075,'Raman HeII 6545',fontdict={'fontsize':'10','fontweight':'bold'})
plt.annotate('',xy=(6543,0.05),xytext=(6538,0.07),arrowprops=dict(arrowstyle='->',color='black',lw=2))
plt.text(6550,0.125,'HeII 6560',fontdict={'fontsize':'10','fontweight':'bold'})
plt.annotate('',xy=(6557.3,0.1),xytext=(6553.3,0.12),arrowprops=dict(arrowstyle='->',color='black',lw=2))
plt.text(6566,0.15, r'H$\alpha$',fontdict={'fontsize':'13','fontweight':'bold'})
plt.title('Raman HeII 6545 of NGC 6741')
plt.grid(linestyle=':')
plt.legend(loc ='upper right')
plt.savefig('6545_NGC6741.png')
#plt.show()

...
df1 = pd.read_csv('cngc6741_3600s.0030.txt', sep='  ',names = ['wavelength','flux']) #beta
...
for i in range(len(df1)):
    if 4849 < ww1[i] and ww1[i] < 4851.6:
        wl_r.append(ww1[i])
        flux_r.append(ff1[i])
    elif 4858.5< ww1[i] and ww1[i] < 4860.6:
        wl_he.append(ww1[i])
        flux_he.append(ff1[i])    
    elif 4860.5 < ww1[i] and ww1[i]<4862.6:
        wl_hb.append(ww1[i])
...
plt.annotate('',xy=(4849.5,0.09),xytext=(4847.3,0.12),arrowprops=dict(arrowstyle='->',color='black',lw=2))
plt.text(4853,0.15,'HeII 4859',fontdict={'fontsize':'10','fontweight':'bold'})
plt.annotate('',xy=(4858.4,0.11),xytext=(4855.5,0.14),arrowprops=dict(arrowstyle='->',color='black',lw=2))
plt.text(4863.3,0.2, r'H$\beta$',fontdict={'fontsize':'13','fontweight':'bold'})
plt.title('Raman HeII 4851 of NGC 6741')
plt.grid(linestyle=':')
plt.xlim(4840,4870)
plt.ylim(0,0.3)
plt.legend(loc = 'upper right') 
plt.savefig('4851_NGC6741.png')
#plt.show()
